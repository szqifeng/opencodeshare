---
description: OpenCode 快速起步，支持 Windows、macOS、Linux 多平台。本文详细介绍如何下载、安装和配置 OpenCode，以及如何验证安装是否成功。
keywords: ["OpenCode 快速起步", "OpenCode 安装", "OpenCode 下载", "如何安装 OpenCode"]
---
...
首次启动 OpenCode 时，会显示配置向导，按步骤完成初始设置：
...
- **简洁直观**：每一步只关注一个配置项
- **视觉明确**：使用 emoji 和图标增强可读性
- **导航清晰**：上一步、下一步、跳过配置等按钮一目了然
- **自适应布局**：自动适应不同屏幕尺寸
- **进度提示**：显示当前步骤（1/5、2/5 等）
...
#### 模型选择
- **GPT-4o (推荐)**：最强大的模型，适合复杂任务
- **GPT-4o Mini**：速度更快，成本更低
- **Claude 3.5 Sonnet**：擅长分析和推理
- **本地模型**：完全离线，隐私保护

#### API Key 配置
- **支持的服务商**：OpenAI、Anthropic、本地模型
- **存储方式**：安全加密存储在本地
- **验证机制**：自动验证 Key 有效性
- **跳过选项**：可以先使用，稍后配置
...
1. **首次推荐配置**：首次使用建议完成配置，以获得完整体验
2. **随时可更改**：配置完成后，随时可在设置中修改
3. **API Key 安全**：API Key 仅存储在本地，不会上传云端
4. **网络要求**：在线模型需要稳定的网络连接
5. **本地模型**：选择本地模型后，可以完全离线使用
...
打开浏览器访问 `http://localhost:3000`，在对话框中输入以下内容：
